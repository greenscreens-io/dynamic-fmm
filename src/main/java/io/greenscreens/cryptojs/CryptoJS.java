...
public interface CryptoJS {
    ...
    /**
     * Automatic pointer release for MemorySegments return values used for automatic Java type conversion
     * @param pointer
     */
    @GarbageCollector
     ...
     * @return 
     * 
     * C/C++ requires size of a pointer. 
     * Annotation here define from which input argument to take size of bytes in return value  
     */
     ...
     * 
     * C/C++ requires size of a pointer.
     * Annotation here contains fixed byte length returned, as we know that MD5 always return 16 bytes.
     */
     ...
     * @param data Data to calculate hash
     * @param length Length of data (required for C/C++ pointer)
     * @param key Always 20 bytes length, so we don't need to pass pointer byte size
     * @return Calculated hash 
     */
     ...
     * @param data Data to calculate hash
     * @param length Length of data (required for C/C++ pointer)
     * @param mac Always 20 bytes length, so we don't need to pass pointer byte size
     * @param key Always 20 bytes length, so we don't need to pass pointer byte size
     * @return Calculated hash 
     */
     ...
     * @param hashLen Size in bytes (1, 28, 32, 48, 64, 65) or bits (20, 224, 256, 384, 512, 521)
     * @return
     */
     ...
     * @param size Size of a hash algorithm  in bytes (1, 28, 32, 48, 64) or bits (20, 224, 256, 384, 512)
     ...
     * @return
     */
    @Size(index = 3) byte[] RSA_Sign_PKCS_1v15(final String id, final byte[] raw, final int len, final int size);
     ...
     * @param hashLength Size in bytes (1, 28, 32, 48, 64, 65) or bits (20, 224, 256, 384, 512, 521)
     * @param saltLength Size of returning salt 
     * @return
     */
    @Size(index = 4) byte[] RSA_Sign_PSS(final String id, final byte[] raw, final int len, final int hashLength, final int saltLength);
     ...
     * @param size Key size in bytes (28, 32, 48, 64, 65) or bits (224, 256, 384, 512, 521)
     * @return Cache ID of a key
     */
